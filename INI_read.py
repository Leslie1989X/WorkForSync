...
# ini文件的正则匹配
    # Job 匹配job文件夹名
def ini_read(file_path:str):
    #file_path = input('输入Job路径: ').replace('"','')
    start = time.time()  
    if len(file_path) == 0:
        pass
    elif file_path.lower() == '*rcai':
        print(1)
        pass
        ...
        if calculate:
            ...
            print(f'文件{machine}_job_details.json, {machine}_jobs.csv, {machine}_exception.txt, {machine}_joblist_py.txt输出在{file_path}下')
            pass
...
if __name__ == '__main__':
    ...
    while len(a) == 0:
        file_path = input('输入Job路径: ').replace('"','')
        ini_read(file_path)
        a.append(input('Press Enter to exit...'))
